...
export default function Admin() {
  ...
  if (!isAdmin) {
    return (
      <div style={{ minHeight: '100vh', background: '#06050e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <AmbientBg />
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <Icon name="shield-off" size={48} color="#ff4d6d" />
          <h2 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 22, color: '#f4f0ff', margin: '20px 0 10px' }}>Access Denied</h2>
          <p style={{ color: '#6e6888', marginBottom: 24 }}>This page is restricted to administrators.</p>
    ...
    <div style={{ minHeight: '100vh', background: '#06050e', display: 'flex', fontFamily: 'DM Sans, sans-serif' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        input:focus { outline: none; border-color: rgba(124,77,255,0.4) !important; }
        ::-webkit-scrollbar { width: 4px; }
        ::-webkit-scrollbar-thumb { background: #2e2a42; border-radius: 2px; }
      `}</style>

      {/* ── Sidebar ──────────────────────────────────────────────────────── */}
      <aside style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 220, background: '#0e0c1a', borderRight: '1px solid rgba(255,255,255,0.05)', zIndex: 10, display: 'flex', flexDirection: 'column', padding: '24px 0' }}>
        <div style={{ padding: '0 20px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
          <LogoMark size={26} />
          <div>
            <div style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontWeight: 700, fontSize: 14, color: '#f4f0ff' }}>Admin Panel</div>
            <div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: '#6e6888' }}>Mentorix.AI</div>
          </div>
        </div>
        ...
        {TABS.map(t => (
          <button key={t.id} className="nav-item" onClick={() => setTab(t.id)} style={{
            ...
            background: tab === t.id ? 'rgba(124,77,255,0.1)' : 'transparent',
            borderLeft: `2px solid ${tab === t.id ? '#7c4dff' : 'transparent'}`,
            border: 'none', cursor: 'pointer', width: '100%', textAlign: 'left',
            color: tab === t.id ? '#f4f0ff' : '#6e6888',
            fontFamily: 'DM Sans, sans-serif', fontSize: 13, fontWeight: 500, transition: 'all 0.15s',
          }}>
            <Icon name={t.icon} size={15} color={tab === t.id ? '#7c4dff' : '#6e6888'} />
            {t.label}
          </button>
        ))}
      ...
      {/* ── Main ─────────────────────────────────────────────────────────── */}
      <main style={{ marginLeft: 220, flex: 1, padding: '32px clamp(24px,4vw,48px)', position: 'relative', zIndex: 1, color: '#c0bbd8' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 24, fontWeight: 700, color: '#f4f0ff', margin: '0 0 4px' }}>
            {TABS.find(t => t.id === tab)?.label}
          </h1>
          <p style={{ margin: 0, fontSize: 13, color: '#6e6888', fontFamily: 'DM Mono, monospace' }}>admin@mentorix.ai</p>
        </div>
            ...
            {tab === 'overview' && currentData && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 16 }}>
                {Object.entries(currentData).map(([k, v]) => (
                  <Card key={k}>
                    <div style={{ fontSize: 11, color: '#6e6888', fontFamily: 'DM Mono, monospace', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>{k.replace(/_/g, ' ')}</div>
                    <div style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 28, fontWeight: 800, color: '#f4f0ff' }}>{String(v)}</div>
                  </Card>
                ))}
              </div>
            )}
              ...
              <div>
                <div style={{ marginBottom: 16, display: 'flex', gap: 10, alignItems: 'center' }}>
                  <input value={search} onChange={e => setSearch(e.target.value)}
                    ...
                    style={{ flex: 1, maxWidth: 340, padding: '9px 14px', background: '#161328', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, color: '#f4f0ff', fontSize: 13, fontFamily: 'DM Sans, sans-serif' }} />
                  <span style={{ fontSize: 13, color: '#6e6888', fontFamily: 'DM Mono, monospace' }}>
                      ...
                      <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                          {['Name','Email','Dept','Year','Institution','Status','Actions'].map(h => (
                            <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontFamily: 'DM Mono, monospace', fontSize: 10, color: '#6e6888', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 500 }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {filterUsers(currentData?.users || []).map(u => (
                          <tr key={u.id || u.email}>
                            {[
                              ...
                              <td key="e" style={{ ...td, fontFamily: 'DM Mono, monospace', fontSize: 12 }}>{u.email}</td>,
                        ...
                        {!filterUsers(currentData?.users || []).length && (
                          <tr><td colSpan={7} style={{ padding: '32px 16px', textAlign: 'center', color: '#6e6888', fontSize: 13 }}>No users found</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                      {(currentData?.sessions || []).map(s => (
                        <tr key={s.id}>
                          <td style={{ ...td, fontFamily: 'DM Mono, monospace', fontSize: 12 }}>{s.email}</td>
                          ...
                          <td style={td}>
                            <span style={{ color: (s.tab_warnings || 0) >= 3 ? '#ff4d6d' : '#6e6888', fontFamily: 'DM Mono, monospace', fontSize: 13, fontWeight: (s.tab_warnings || 0) >= 3 ? 700 : 400 }}>
                              {s.tab_warnings || 0}
                            </span>
                          </td>
                      ...
                      {!(currentData?.sessions?.length) && (
                        <tr><td colSpan={7} style={{ padding: '32px 16px', textAlign: 'center', color: '#6e6888', fontSize: 13 }}>No sessions yet</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {(currentData?.leaderboard || currentData?.honor || []).map((u, i) => (
                  <Card key={u.email} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: i === 0 ? 'rgba(245,166,35,0.15)' : i === 1 ? 'rgba(192,192,192,0.15)' : 'rgba(205,127,50,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'DM Mono, monospace', fontWeight: 700, color: i === 0 ? '#f5a623' : i === 1 ? '#c0c0c0' : '#cd7f32', fontSize: 14, flexShrink: 0 }}>
                      {i + 1}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 600, color: '#f4f0ff', fontSize: 14 }}>{u.name || u.email}</div>
                      <div style={{ fontSize: 12, color: '#6e6888', fontFamily: 'DM Mono, monospace' }}>{u.department || '—'} · {u.institution_name || 'Independent'}</div>
                    </div>
                    <div style={{ fontFamily: 'Bricolage Grotesque, sans-serif', fontSize: 22, fontWeight: 800, color: i === 0 ? '#f5a623' : '#7c4dff' }}>
                ...
                {!(currentData?.leaderboard?.length || currentData?.honor?.length) && (
                  <Card style={{ textAlign: 'center', padding: 48 }}>
                    <p style={{ color: '#6e6888' }}>No honor data yet</p>
                  </Card>
                )}
              </div>
            )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {(currentData?.institutions || []).map(inst => (
                    <Card key={inst.id} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 10, background: 'rgba(124,77,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Icon name="building" size={20} color="#7c4dff" />
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, color: '#f4f0ff', fontSize: 14, marginBottom: 3 }}>{inst.name}</div>
                        <div style={{ fontSize: 12, color: '#6e6888', fontFamily: 'DM Mono, monospace' }}>{inst.contact_email || '—'}</div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <Btn size="sm" variant="ghost" onClick={() => toggleInst(inst.id)}>
                          <Icon name={inst.env === 'prod' ? 'toggle-right' : 'toggle-left'} size={15} color={inst.env === 'prod' ? '#00e5b4' : '#6e6888'} />
                        </Btn>
                  ...
                  {!(currentData?.institutions?.length) && (
                    <Card style={{ textAlign: 'center', padding: 48 }}>
                      ...
                      <p style={{ color: '#6e6888', marginTop: 16 }}>No institutions yet. Add one above.</p>
                    </Card>
                  )}
                </div>
              </div>
...
const td = { padding: '12px 16px', fontSize: 13, color: '#c0bbd8', borderBottom: '1px solid rgba(255,255,255,0.04)' }
const labelStyle = { display: 'block', fontFamily: 'DM Mono, monospace', fontSize: 10, color: '#6e6888', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 6 }
const inputStyle = { width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: '#161328', color: '#f4f0ff', fontSize: 14, fontFamily: 'DM Sans, sans-serif' }
