  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = 0; i < bars; i++) {
        ...
        grad.addColorStop(1, 'rgba(0,229,180,0.5)')
        ctx.fillStyle = grad
        ctx.beginPath()
    ...
    <div style={{ minHeight:'100vh', background:'#06050e', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24 }}>
      ...
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        textarea:focus { outline:none; border-color:rgba(124,77,255,0.5)!important; }
      `}</style>
      ...
      <div style={{ position:'relative', zIndex:1, width:'100%', maxWidth:600 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:32 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <LogoMark size={24} />
            <span style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontWeight:700, fontSize:15, color:'#f4f0ff' }}>Voice Session</span>
          </div>
        ...
        {/* Main card */}
        <div style={{ background:'#0e0c1a', border:'1px solid rgba(255,255,255,0.07)', borderRadius:20, padding:'clamp(28px,5vw,40px)' }}>
          ...
          {phase === 'intro' && (
            <div style={{ textAlign:'center' }}>
              ...
              <h2 style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:24, fontWeight:700, color:'#f4f0ff', margin:'0 0 12px' }}>Ready to Begin?</h2>
              <p style={{ color:'#6e6888', fontSize:14, lineHeight:1.7, margin:'0 0 12px' }}>
                You'll be asked up to <strong style={{ color:'#f4f0ff' }}>{Math.min(MAX_Q, questions.length)} questions</strong> by your AI mentor.
                Speak clearly or use type mode. Stay on this tab during the session.
              </p>
              <p style={{ color:'#6e6888', fontSize:12, fontFamily:'DM Mono, monospace', margin:'0 0 32px' }}>
              ...
              <div style={{ display:'flex', gap:12, justifyContent:'center', marginBottom:24 }}>
                <button onClick={() => setInputMode('voice')} style={{
                  ...
                  background: inputMode==='voice' ? 'rgba(124,77,255,0.12)' : 'transparent',
                  border: `1px solid ${inputMode==='voice' ? 'rgba(124,77,255,0.4)' : 'rgba(255,255,255,0.07)'}`,
                  color: inputMode==='voice' ? '#f4f0ff' : '#6e6888',
                  fontFamily:'DM Sans, sans-serif', fontSize:13, transition:'all 0.15s',
                }}>
                  <Icon name="mic" size={14} color={inputMode==='voice' ? '#7c4dff' : '#6e6888'} /> Voice Mode
                </button>
                <button onClick={() => setInputMode('type')} style={{
                  padding:'10px 20px', borderRadius:10, cursor:'pointer',
                  background: inputMode==='type' ? 'rgba(124,77,255,0.12)' : 'transparent',
                  border: `1px solid ${inputMode==='type' ? 'rgba(124,77,255,0.4)' : 'rgba(255,255,255,0.07)'}`,
                  color: inputMode==='type' ? '#f4f0ff' : '#6e6888',
                  fontFamily:'DM Sans, sans-serif', fontSize:13, transition:'all 0.15s',
                }}>
                  <Icon name="keyboard" size={14} color={inputMode==='type' ? '#7c4dff' : '#6e6888'} /> Type Mode
                </button>
              </div>

              <Btn onClick={startSession} size="lg" fullWidth style={{ background:'linear-gradient(135deg,#7c4dff,#5530cc)', boxShadow:'0 4px 32px rgba(124,77,255,0.3)' }}>
          ...
          {(phase === 'question' || phase === 'listening') && (
            <div>
              ...
              <p style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:18, fontWeight:600, color:'#f4f0ff', lineHeight:1.5, margin:'0 0 24px' }}>
                {questions[qIndex]}
              </p>

              {phase === 'question' && (
                <div style={{ textAlign:'center', paddingTop:8 }}>
                  <div style={{ animation:'pulse 2s infinite', display:'inline-flex', alignItems:'center', gap:6, color:'#6e6888', fontSize:13, fontFamily:'DM Mono, monospace' }}>
                ...
                <div>
                  {inputMode === 'voice' ? (
                    <div style={{ textAlign:'center' }}>
                      ...
                      <div style={{ background:'#161328', borderRadius:12, padding:'14px 18px', marginBottom:20, minHeight:80, textAlign:'left' }}>
                        <p style={{ margin:0, color: transcript ? '#f4f0ff' : '#6e6888', fontSize:14, lineHeight:1.6, fontFamily:'DM Sans, sans-serif', fontStyle: transcript ? 'normal' : 'italic' }}>
                    ...
                    <div>
                      <textarea value={typedAnswer} onChange={e => setTypedAnswer(e.target.value)}
                        placeholder="Type your answer here..." rows={5} style={{
                          width:'100%', padding:'14px 16px', background:'#161328',
                          border:'1px solid rgba(255,255,255,0.07)', borderRadius:12,
                          color:'#f4f0ff', fontSize:14, lineHeight:1.6,
                          fontFamily:'DM Sans, sans-serif', resize:'vertical', marginBottom:16,
                        }} />
          ...
          {phase === 'done' && (
            <div style={{ textAlign:'center', padding:'8px 0' }}>
              <div style={{ width:72, height:72, borderRadius:'50%', background:'rgba(0,229,180,0.1)', border:'1px solid rgba(0,229,180,0.3)', margin:'0 auto 24px', display:'flex', alignItems:'center', justifyContent:'center' }}>
                <Icon name="check-circle" size={32} color="#00e5b4" />
              </div>
              <h2 style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:24, fontWeight:700, color:'#f4f0ff', margin:'0 0 12px' }}>Session Complete!</h2>
              <p style={{ color:'#6e6888', fontSize:14, margin:'0 0 8px' }}>
                You answered <strong style={{ color:'#f4f0ff' }}>{answers.length} questions</strong>
              </p>
              <p style={{ color:'#6e6888', fontSize:12, fontFamily:'DM Mono, monospace', margin:'0 0 32px' }}>
        ...
        {phase === 'listening' && (
          <div style={{ textAlign:'center', marginTop:16 }}>
            <button onClick={() => endSession(false)} style={{ background:'none', border:'none', cursor:'pointer', color:'#2e2a42', fontSize:12, fontFamily:'DM Mono, monospace' }}>
              End session early
            </button>
          </div>
        )}
