...
export default function Assessment() {
  ...
  const riskColor = r => r === 'Low' ? '#059669' : r === 'Medium' ? '#2563EB' : '#DC2626'
    ...
    <div style={{ minHeight: '100vh', background: '#F8F9FC', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        .opt:hover { border-color: rgba(37,99,235,0.3) !important; cursor: pointer; }
      ...
      <div style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 580 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <LogoMark size={24} />
            <span style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, fontSize: 15, color: '#0F172A' }}>Assessment</span>
          </div>
          {phase === 'quiz' && (
            <div style={{ fontFamily: 'Inter, monospace', fontSize: 14, color: timeLeft < 60 ? '#DC2626' : '#94A3B8', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Icon name="clock" size={14} color={timeLeft < 60 ? '#DC2626' : '#94A3B8'} />
              {fmt(timeLeft)}
            </div>
          )}
        ...
        {phase === 'loading' && (
          <Card style={{ textAlign: 'center', padding: 56 }}>
            <Spinner size={36} />
            <p style={{ color: '#94A3B8', marginTop: 18, fontSize: 14 }}>Loading questions{dept ? ` for ${dept}` : ''}…</p>
          </Card>
        )}
        ...
        {phase === 'error' && (
          <Card style={{ textAlign: 'center', padding: 56 }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.2)', margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Icon name="alert-circle" size={30} color="#ff4d6d" />
            </div>
            <h3 style={{ fontFamily: 'Inter, sans-serif', fontSize: 20, color: '#0F172A', marginBottom: 8 }}>Assessment Unavailable</h3>
            <p style={{ color: '#94A3B8', fontSize: 14, marginBottom: 28, lineHeight: 1.6 }}>Could not load questions. The backend may be starting up, or no questions are available for your department.</p>
        ...
        {phase === 'intro' && (
          <Card className="fade-up">
            <div style={{ textAlign: 'center' }}>
              ...
              <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: 26, fontWeight: 700, color: '#0F172A', margin: '0 0 10px' }}>Academic Assessment</h2>
              <p style={{ color: '#94A3B8', fontSize: 14, margin: '0 0 32px', lineHeight: 1.75, maxWidth: 380, marginLeft: 'auto', marginRight: 'auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 32 }}>
                {[
                  ...
                  <div key={s.label} style={{ padding: '16px 10px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(0,0,0,0.04)', textAlign: 'center' }}>
                    <Icon name={s.icon} size={18} color="#6e6888" />
                    <div style={{ fontFamily: 'Inter, monospace', fontSize: 16, fontWeight: 600, color: '#0F172A', marginTop: 8 }}>{s.val}</div>
                    <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 3 }}>{s.label}</div>
                  </div>
                ))}
              </div>

              <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.15)', marginBottom: 28, textAlign: 'left' }}>
        ...
        {phase === 'quiz' && q && (
          <div className="fade-up" key={current}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <span style={{ fontFamily: 'Inter, monospace', fontSize: 12, color: '#94A3B8' }}>
            ...
            <div style={{ height: 3, background: 'rgba(0,0,0,0.04)', borderRadius: 2, marginBottom: 20, overflow: 'hidden' }}>
            ...
            <Card>
              {/* Domain badge */}
              {q.domain_label && (
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginBottom: 16, padding: '4px 12px', borderRadius: 100, background: 'rgba(37,99,235,0.08)', border: '1px solid rgba(37,99,235,0.2)' }}>
                  <span style={{ fontFamily: 'Inter, monospace', fontSize: 10, color: '#a87fff', letterSpacing: '0.06em' }}>{q.domain_label.toUpperCase()}</span>
                </div>
              )}

              <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 17, fontWeight: 600, color: '#0F172A', lineHeight: 1.55, margin: '0 0 22px' }}>
                ...
                {(q.options || []).map((opt, idx) => {
                  ...
                  return (
                    <button key={idx} className="opt" onClick={() => selectAnswer(q.id, idx)} style={{
                      ...
                      background: selected ? 'rgba(37,99,235,0.1)' : '#F8F9FC',
                      border: `1px solid ${selected ? 'rgba(37,99,235,0.45)' : 'rgba(0,0,0,0.07)'}`,
                      ...
                      <div style={{
                        ...
                        border: `2px solid ${selected ? '#2563EB' : 'rgba(255,255,255,0.15)'}`,
            ...
            <div style={{ display: 'flex', gap: 5, justifyContent: 'center', marginTop: 16, flexWrap: 'wrap' }}>
              {questions.map((qq, i) => (
                <button key={i} className="dot-btn" onClick={() => setCurrent(i)} style={{
                  ...
                  fontFamily: 'Inter, monospace', color: '#0F172A',
                  background: answers[qq.id] !== undefined
                    ? '#2563EB'
                    : i === current
                      ? 'rgba(37,99,235,0.3)'
                      : 'rgba(0,0,0,0.04)',
                  boxShadow: i === current ? '0 0 0 2px rgba(37,99,235,0.4)' : 'none',
        ...
        {phase === 'submitting' && (
          <Card style={{ textAlign: 'center', padding: 56 }}>
            <Spinner size={36} />
            <p style={{ color: '#94A3B8', marginTop: 18, fontSize: 14 }}>Analysing your responses with AI…</p>
          </Card>
        )}
        ...
        {phase === 'result' && result && (
          <div className="fade-up">
            ...
            <Card style={{ marginBottom: 16, textAlign: 'center' }}>
              <h2 style={{ fontFamily: 'Inter, sans-serif', fontSize: 24, fontWeight: 700, color: '#0F172A', margin: '0 0 6px' }}>Assessment Complete</h2>
              <p style={{ color: '#94A3B8', fontSize: 13, margin: '0 0 28px' }}>Your academic risk evaluation</p>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 24, alignItems: 'center', marginBottom: 28, flexWrap: 'wrap' }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 11, color: '#94A3B8', fontFamily: 'Inter, monospace', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Risk Level</div>
                    <Badge color={riskBadge(result.risk_level)}>{result.risk_level || 'Unknown'}</Badge>
                  </div>
                  <div style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 11, color: '#94A3B8', fontFamily: 'Inter, monospace', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Trend</div>
                    <span style={{ fontSize: 14, color: '#0F172A', fontFamily: 'Inter, monospace' }}>{result.trend || '—'}</span>
                  </div>
                  {result.recommendation?.track && (
                    <div>
                      <div style={{ fontSize: 11, color: '#94A3B8', fontFamily: 'Inter, monospace', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>Recommended Track</div>
                      <span style={{ fontSize: 14, color: '#2563EB', fontWeight: 600, fontFamily: 'Inter, monospace' }}>{result.recommendation.track}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              {result.ai_explanation && (
                <div style={{ padding: '16px 18px', borderRadius: 12, background: 'rgba(37,99,235,0.06)', border: '1px solid rgba(37,99,235,0.15)', marginBottom: 24, textAlign: 'left' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    <Icon name="zap" size={14} color="#7c4dff" />
                    <span style={{ fontFamily: 'Inter, monospace', fontSize: 11, color: '#2563EB', textTransform: 'uppercase', letterSpacing: '0.06em' }}>AI Insight</span>
                  </div>
                  <p style={{ margin: 0, fontSize: 13, color: '#334155', lineHeight: 1.7 }}>{result.ai_explanation.slice(0, 400)}{result.ai_explanation.length > 400 ? '…' : ''}</p>
                </div>
              )}
              ...
              {result.recommendation?.courses?.length > 0 && (
                <div style={{ marginBottom: 24, textAlign: 'left' }}>
                  <div style={{ fontFamily: 'Inter, monospace', fontSize: 11, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Recommended Courses</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {result.recommendation.courses.slice(0, 3).map((c, i) => (
                      <div key={i} style={{ padding: '11px 14px', borderRadius: 10, background: '#F8F9FC', border: '1px solid rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <Icon name="book-open" size={14} color="#7c4dff" />
                        <span style={{ flex: 1, fontSize: 13, color: '#0F172A' }}>{c.title || c}</span>
                        {c.url && <a href={c.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 11, color: '#2563EB', fontFamily: 'Inter, monospace', textDecoration: 'none' }}>Open →</a>}
                      </div>
                    ))}
                  </div>
                </div>
