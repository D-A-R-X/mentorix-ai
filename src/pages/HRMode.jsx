...
export default function HRMode() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8F9FC', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, position:'relative' }}>
      <AmbientBg />
      <style>{`
      ...
      <div style={{ position:'relative', zIndex:1, width:'100%', maxWidth:680 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <LogoMark size={24} />
            <span style={{ fontFamily:'Inter, sans-serif', fontWeight:700, fontSize:15, color:'#0F172A' }}>HR Mode</span>
          </div>
        ...
        {phase === 'intro' && (
          <Card>
            <div style={{ textAlign:'center' }}>
              <div style={{ width:80, height:80, borderRadius:'50%', background:'rgba(5,150,105,0.08)', border:'1px solid rgba(5,150,105,0.2)', margin:'0 auto 24px', display:'flex', alignItems:'center', justifyContent:'center' }}>
                <Icon name="briefcase" size={36} color="#00e5b4" />
              </div>
              <h2 style={{ fontFamily:'Inter, sans-serif', fontSize:24, fontWeight:700, color:'#0F172A', margin:'0 0 12px' }}>HR Interview Simulation</h2>
              <p style={{ color:'#94A3B8', fontSize:14, lineHeight:1.7, margin:'0 0 24px', maxWidth:420, marginLeft:'auto', marginRight:'auto' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:8, marginBottom:32 }}>
                {CATEGORIES.map(c => (
                  <div key={c} style={{ padding:'10px 8px', borderRadius:10, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(0,0,0,0.04)', textAlign:'center' }}>
                    <div style={{ fontSize:11, color:'#94A3B8', fontFamily:'Inter, monospace' }}>{c}</div>
                  </div>
                ))}
              </div>

              <Btn onClick={startSession} size="lg" fullWidth style={{ background:'linear-gradient(135deg,#00e5b4,#00b48f)', color:'#F8F9FC' }}>
                <Icon name="video" size={16} color="#06050e" /> Begin Interview
              </Btn>
              <p style={{ fontSize:11, color:'#CBD5E1', marginTop:12, fontFamily:'Inter, monospace' }}>
        ...
        {phase === 'session' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:16, alignItems:'start' }}>
            <Card>
              {/* Progress */}
              <div style={{ height:3, background:'rgba(0,0,0,0.04)', borderRadius:2, marginBottom:24, overflow:'hidden' }}>
              ...
              <p style={{ fontFamily:'Inter, sans-serif', fontSize:18, fontWeight:600, color:'#0F172A', lineHeight:1.5, margin:'0 0 20px' }}>
                {questions[qIndex]}
              </p>

              <div style={{ background:'#F8F9FC', borderRadius:12, padding:'14px 16px', minHeight:100, marginBottom:20 }}>
                <p style={{ margin:0, color: transcript ? '#0F172A' : '#94A3B8', fontSize:14, lineHeight:1.6, fontStyle: transcript ? 'normal' : 'italic' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div style={{ width:180, borderRadius:14, overflow:'hidden', border:`1px solid ${cameraOn ? 'rgba(5,150,105,0.3)' : 'rgba(0,0,0,0.07)'}`, position:'relative' }}>
                <video ref={videoRef} autoPlay muted playsInline style={{ width:'100%', display:'block', background:'#F8F9FC' }} />
                {!cameraOn && (
                  <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:'#F8F9FC' }}>
                    <Icon name="camera-off" size={24} color="#2e2a42" />
                  </div>
                )}
                {cameraOn && <PostureMonitor videoRef={videoRef} onScore={setPostureScore} />}
              </div>
              <div style={{ padding:'10px 14px', borderRadius:10, background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', textAlign:'center' }}>
                <div style={{ fontSize:11, color:'#94A3B8', fontFamily:'Inter, monospace', marginBottom:4 }}>POSTURE</div>
                <div style={{ fontFamily:'Inter, sans-serif', fontSize:20, fontWeight:700, color: postureScore >= 70 ? '#059669' : '#DC2626' }}>
        ...
        {phase === 'result' && (
          <div>
            <Card style={{ marginBottom:16, textAlign:'center' }}>
              <h2 style={{ fontFamily:'Inter, sans-serif', fontSize:22, fontWeight:700, color:'#0F172A', margin:'0 0 8px' }}>Interview Complete</h2>
              <p style={{ color:'#94A3B8', fontSize:13, margin:'0 0 28px' }}>{saving ? 'Saving results...' : 'Results saved to your profile'}</p>
              <div style={{ display:'flex', justifyContent:'center', marginBottom:28 }}>
                <ScoreRing score={Math.round(overallScore)} size={130} label="Overall" />
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', gap:10 }}>
                {CATEGORIES.map(c => (
                  <div key={c} style={{ padding:'14px 8px', borderRadius:12, background:'#F8F9FC', border:'1px solid rgba(0,0,0,0.04)', textAlign:'center' }}>
                    <div style={{ fontFamily:'Inter, monospace', fontSize:16, fontWeight:600, color: scores[c] >= 70 ? '#059669' : scores[c] >= 50 ? '#2563EB' : '#DC2626' }}>
                      {Math.round(scores[c])}
                    </div>
                    <div style={{ fontSize:10, color:'#94A3B8', marginTop:4 }}>{c}</div>
                  </div>
                ))}
              </div>
            </Card>
