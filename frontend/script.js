...
const API_BASE_URL = (window.MENTORIX_API_BASE_URL || "").trim();
const ANALYZE_ENDPOINT = API_BASE_URL
  ? `${API_BASE_URL.replace(/\/$/, "")}/analyze-risk`
  : "/analyze-risk";
...
async function analyze() {
  ...
  try {
    ...
    if (!res.ok) {
      const errorText = result?.detail || result?.errors?.join("; ") || "Unable to analyze right now. Please try again.";
